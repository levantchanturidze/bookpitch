...
- **~~`sentryBeforeSend` has no SDK~~** — corrected 2026-08-23 (P17-007). `@sentry/nextjs` 10.67.0 **is** a dependency and has been since Phase 8; the claim was stale. `sentryBeforeSend` is called by `sentry.server.config.ts` and `sentry.edge.config.ts`, and the browser config uses `scrubSensitive` from `lib/scrub.ts` directly (importing the logger pulls `node:async_hooks` into the client bundle and fails the build). What is genuinely absent is the **DSN**: every `Sentry.init()` is inside `if (…_DSN)`, and production has neither `SENTRY_DSN` nor `NEXT_PUBLIC_SENTRY_DSN`, so nothing reports. Tracked by monitor check `production-observability-unconfigured`.
...
- **Insurance claim export UI vs. backend.** `/settings/insurance` renders and calls `lib/insurance.ts::listInsurers`. ~~There is no "add insurer" UI; insurers are created via direct DB writes.~~ Corrected 2026-08-23 (P17-015): `AddInsurerForm` in `app/(app)/settings/insurance/InsuranceView.tsx` assigns an insurer and policy number to a patient via `PATCH /api/customers/[id]`, which is how insurers come into existence — `listInsurers` returns the distinct `customers.insurerName` values. There is no separate insurers table, and no direct DB write is required. What remains true: there is no way to manage an insurer as an entity of its own (rename, merge, delete).
- **~~Ownership-transfer UI absent~~** — corrected 2026-08-23 (P17-015). `app/(app)/settings/ownership/` renders `components/settings/OwnershipPanel.tsx`, which surfaces nomination and the accept/decline actions against the same endpoints (`POST /api/admin/ownership-transfer`, `/[id]/accept`, `/[id]/decline`, `DELETE /[id]`). The nominee still gets an email and an in-app notification.
...
**Permissions seeded in `prisma/rbac-seed.ts` with no authorization callsite anywhere in the codebase.** Each is tagged `notYetImplemented: '<bundle_slug>'` in the seed file, and `scripts/check-orphan-perms.ts` (wired into `npm test`) fails CI if a PR adds a seeded permission without either a callsite or a bundle tag — closing the "grant without check" class SEC-008 belonged to.

Recomputed 2026-08-23 (P17-006), from HEAD rather than from an earlier count: **67 seeded = 40 enforced + 27 explicitly deferred, 0 orphan, 0 inconsistent.** The checker previously counted a permission as enforced if the string `perm('x')` appeared anywhere, which meant five keys were "enforced" purely by appearing in the impersonation **deny** list — the opposite of being enforced — while genuine callsites written as `can(v.ctx, …)` were missed entirely. It now parses with the TypeScript compiler and counts only `requirePermission()`, `can()` and `ctx.{permissions,platformPermissions}.has()`. Details in `docs/phase-17-stabilization-ledger.md` §2.

The list below predates that recount and describes the *features* that are absent, which is still accurate; the per-key classification is authoritative in the seed file and the checker's output.
...
- ~~§7.2 break-glass 2FA — implementation is password-only~~ — corrected 2026-08-23 (P17-015). `lib/platform/break-glass.ts::startBreakGlass` requires exactly one of `totpCode` or `recoveryCode` and rejects a request carrying neither or both; `preCheckTotp` also records `mfa_last_totp_window` so a code cannot be replayed. Line 185 of this same document already described it correctly — the two statements had been contradicting each other
- ~~Audit-log CSV export — no export button exists~~ — corrected 2026-08-23 (P17-015). `components/audit/AuditView.tsx:70` renders an "Export CSV" link to `/api/audit/export`, carrying the viewer's active filters; the endpoint is guarded by `audit.read` and capped at 5000 rows
