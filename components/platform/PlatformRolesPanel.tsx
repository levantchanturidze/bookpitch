...
export default function PlatformRolesPanel({
  ...
  async function submit(revoke: boolean) {
    ...
    try {
      ...
      setMsg({ ok: true, text: `${revoke ? 'Revoked' : 'Assigned'} — previous role: ${j.previousRoleKey ?? '(none)'}` });
      setEmail('');
      router.refresh();
    } catch (e) {
            ...
            {holders.map((h) => (
              <tr key={h.userId} className="hover:bg-slate-800/40">
                ...
                <td className="px-2 py-2">
                  <span className="rounded-md bg-purple-900 px-1.5 py-0.5 font-mono text-[10px] uppercase text-purple-200">
                    {h.roleKey}
                  </span>
                </td>
          ...
          <div className="space-y-3">
            <label className="block">
              ...
              <span className="mt-1 block text-[11px] text-slate-500">
                User must already exist. Bootstrap SUPER_ADMINs via <code>scripts/create-platform-user.ts</code>.
              </span>
            </label>
            <label className="block">
              <span className="mb-1 block text-xs font-semibold text-slate-300">Role</span>
              <select value={roleKey} onChange={(e) => setRoleKey(e.target.value as never)}
                className="w-full rounded-md border border-slate-700 bg-slate-950 px-3 py-2 text-sm text-slate-100">
                {PLATFORM_ROLE_KEYS.map((k) => <option key={k} value={k}>{k}</option>)}
              </select>
            </label>
            <div className="flex gap-2 pt-1">
              <button
                onClick={() => submit(false)} disabled={busy || !email.trim()}
                className="rounded-md bg-emerald-600 px-3 py-2 text-xs font-semibold text-white hover:bg-emerald-500 disabled:opacity-50">
                {busy ? '…' : 'Assign role'}
              </button>
              <button
                onClick={() => submit(true)} disabled={busy || !email.trim()}
            ...
            {msg && (
              <div className={`mt-2 rounded-md p-2 text-xs ${msg.ok ? 'bg-emerald-950 text-emerald-200' : 'bg-red-950 text-red-200'}`}>
                {msg.text}
              </div>
            )}
          </div>
