...
describe('probe authorisation is derived, scoped and short-lived', () => {
  it('a freshly issued token verifies', () => {
    const t = issueProbeToken(SECRET, 'nonce-abc12345', NOW);
  ...
  it('is bound to ONE nonce — it cannot authorise a different probe', () => {
    const t = issueProbeToken(SECRET, 'nonce-abc12345', NOW);
    const v = verifyProbeToken(SECRET, { ...t, nonce: 'nonce-different' }, NOW + 1_000);
  ...
  it('a token signed with a different secret does not verify', () => {
    const t = issueProbeToken('some-other-secret', 'nonce-abc12345', NOW);
  ...
  it('malformed input is refused, not thrown on', () => {
    for (const bad of [
      ...
      { nonce: 'nonce-abc12345', token: 'not-hex', expiresAt: NOW + 1000 },
      { nonce: 'nonce-abc12345', token: 'a'.repeat(64), expiresAt: 'soon' },
