...
describe('/api/platform/impersonate — start / end / restricted perms / expiry', () => {
  ...
  beforeAll(async () => {
    ...
    const admin = await prismaAdmin.appUser.findUniqueOrThrow({
  ...
  beforeEach(async () => {
    ...
    await prismaAdmin.impersonationSession.deleteMany({ where: { actorUserId: platformAdminId } });
  ...
  it('start succeeds when flag is enabled + audit row written + sessionVersion bumped', async () => {
    ...
    // Session row exists.
    const session = await prismaAdmin.impersonationSession.findUniqueOrThrow({
    ...
    const audit = await prismaAdmin.auditLog.findFirst({
  ...
  it('RESTRICTED perms deny during impersonation (spec §7.1 rule 5)', async () => {
    ...
    authMock.mockResolvedValue({
      user: {
        ...
        platformRoleId: (await prismaAdmin.appUser.findUniqueOrThrow({
  ...
  it('end marks the session ended + bumps sessionVersion', async () => {
    const active = await prismaAdmin.impersonationSession.create({
      data: {
        actorUserId: platformAdminId,
        onBehalfOfUserId: targetUserId,
        organizationId: orgId,
    ...
    expect(res.status).toBe(200);
    const after = await prismaAdmin.impersonationSession.findUniqueOrThrow({ where: { id: active.id } });
