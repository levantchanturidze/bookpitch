...
test.describe('scheduler', () => {
  ...
  test('@journey renders the calendar for a front-desk session', async ({
    page,
  }) => {
  ...
  test('@journey the patient list renders for a front-desk session', async ({
    page,
  }) => {
...
test.describe('scheduler reschedule workflow', () => {
  ...
  test('@journey keeps its own slot visible and moves through the canonical update path', async ({
    ...
    await expect(
      panel.getByLabel('Appointment').locator('option'),
    ).not.toHaveCount(0);

    const currentTime = (
      await panel.getByTestId('reschedule-current-time').textContent()
    )?.trim();
    ...
    await expect(
      timeSelect.locator(`option[value="${currentTime}"]`),
    ).toHaveCount(1);

    const values = await timeSelect.locator('option').evaluateAll((options) =>
    ...
    await expect(
      panel.getByText('Appointment rescheduled successfully.'),
    ).toBeVisible();
  });
});
...
test.describe('public booking widget', () => {
  ...
  test('@journey an unknown slug does not leak whether the location exists', async ({
    page,
  }) => {
    const res = await page.goto('/book/definitely-not-a-real-slug-17');
    expect(res?.status()).toBeGreaterThanOrEqual(200);
    await expect(
      page.getByText(/at .*prisma|stack trace|internal server error/i),
    ).toHaveCount(0);
  });
});
