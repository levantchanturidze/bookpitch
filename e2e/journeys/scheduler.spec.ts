...
test.describe('scheduler', () => {
  ...
  test('@journey navigation between two authorised surfaces keeps the session', async ({ page }) => {
    await page.goto('/scheduler');
    await expect(page.locator('#main')).toBeVisible();
    await page.goto('/patients');
    await expect(page.locator('#main')).toBeVisible();
...
test.describe('scheduler reschedule workflow', () => {
  ...
  test('@journey keeps its own slot visible and moves through the canonical update path', async ({ page }) => {
    await page.goto('/scheduler?month=2026-07');

    const panel = page.getByRole('region', { name: 'Reschedule Appointment' });
