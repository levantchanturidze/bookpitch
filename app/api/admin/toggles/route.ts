...
export async function PATCH(req: NextRequest) {
  return withApi(async () => {
    ...
    if ('providerFinancialReports' in body)
      patch.providerFinancialReports = coerceBool(body.providerFinancialReports);
    if ('providerClinicalNotesOthers' in body)
      patch.providerClinicalNotesOthers = coerceBool(body.providerClinicalNotesOthers);
    if ('frontdeskClientFullHistory' in body)
      patch.frontdeskClientFullHistory = coerceBool(body.frontdeskClientFullHistory);
    if ('frontdeskDiscountCeiling' in body)
      patch.frontdeskDiscountCeiling = coerceNum(body.frontdeskDiscountCeiling);
